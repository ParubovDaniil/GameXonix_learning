import java.awt.*;
import java.awt.event.*;//обработка нажатия клавиатуры
import javax.swing.*;// обработка изображения
import java.util.*;
...
class GameXonix extends JFrame {
    ...
    GameXonix() {
        ...
        setResizable(false);// не будем менять размер
        board.setFont(font);// на черном фоне
        board.setOpaque(true);
        board.setBackground(Color.black);// выключает прозрачность панелей
        board.setForeground(Color.white);
        board.setHorizontalAlignment(JLabel.CENTER);
        add(BorderLayout.CENTER, canvas);// где отображается и прорисовывается наше окно
